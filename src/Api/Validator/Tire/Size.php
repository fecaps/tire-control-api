...
class Size implements ValidatorInterface
{
    ...
    public function validate(array $data)
    {
        ...
        if (mb_strlen($data[$field]) < self::SIZE_MIN_LEN) {
            $exception->addMessage($field, sprintf(TireMessages::LESS_THAN, self::SIZE_MIN_LEN));
            throw $exception;
        }

        if (mb_strlen($data[$field]) > self::SIZE_MAX_LEN) {
            $exception->addMessage($field, sprintf(TireMessages::MORE_THAN, self::SIZE_MAX_LEN));
            throw $exception;
        }
    }
}
